    ...
    <div class="action-container action-bar-fixed">
      <v-row class="fill-height ma-0">
        <v-col class="d-flex justify-end align-center py-2">
          ...
          <DeleteTypeStage :typeStage="typeStageRef" class="ma-2" />
          <v-btn
            class="ma-2"
            color="primary"
            min-width="150"
...
const typeStageRef = ref(route.params.typeStage || {});

// --- État Réactif ---
const validCreateTypeStage = ref(true);
const formCreateTypeStage = ref(null);
...
const submit = async () => {
  ...
  if (formValid) {
    NProgress.start();
    store
      .dispatch('typeStage/editTypeStage', typeStageRef.value)
      .then(() => {
        router.push({ name: 'Type-Stages' });
      })
      .catch((error) => {
          console.error("Erreur de sauvegarde du type de stage:", error);
      })
      .finally(() => {
          NProgress.done();
      });
