...
const formDeleteTypeEntreprise = ref(null)

function closeDialog() {
  dialog.value = false
}

function deleteTypeEntreprise() {
  NProgress.start()
  store
    .dispatch(
      'typeEntreprise/deleteTypeEntreprise',
      props.typeEntreprise.typeEntrepriseId
    )
    .then(() => {
    ...
    <v-dialog v-model="dialog" max-width="650">
      ...
      <v-card rounded="lg" class="pa-2">
        ...
        <v-card-title class="py-4">
          ...
          <span class="text-h6 font-weight-bold">Supprimer une catégorie</span>
        </v-card-title>
        ...
        <v-card-text class="pt-6">
          ...
          <v-card
            class="mb-4"
            v-if="props.typeEntreprise.entrepris.length > 0"
          >
            <v-list disabled>
              <v-subheader>
                Supprimez d'abord les entreprises liées à cette catégorie.
              </v-subheader>

              <v-list-item
                v-for="(entreprise, i) in props.typeEntreprise.entrepris"
                :key="i"
              >
                <v-list-item-title>
                  {{ entreprise.nom }}
