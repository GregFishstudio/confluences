...
function deleteTypeMoyen() {
  NProgress.start()

  store
    .dispatch(
      'typeMoyen/deleteTypeMoyen',
      props.typeMoyen.typeMoyenId
    )
    .then(() => {
    ...
    <v-dialog v-model="dialog" max-width="650">
      ...
      <v-card rounded="lg" class="pa-1">
        ...
        <v-card-title class="py-4">
          ...
          <span class="text-h6 font-weight-bold">Supprimer une communication</span>
        </v-card-title>
        ...
        <v-card-text class="pt-6">
          ...
          <!-- Si le moyen est utilisé -->
          <v-card
            class="mb-2"
            v-if="props.typeMoyen.entrepris.length > 0"
          >
            <v-list disabled>
              <v-subheader>
                Cette communication est liée aux entreprises suivantes :
              </v-subheader>

              <v-list-item
                v-for="(entreprise, i) in props.typeMoyen.entrepris"
                :key="i"
              >
                <v-list-item-title>
                  {{ entreprise.nom }}
