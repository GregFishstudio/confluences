...
function deleteTypeOffre() {
  NProgress.start()

  store
    .dispatch(
      'typeOffre/deleteTypeOffre',
      props.typeOffre.typeOffreId
    )
    .then(() => {
    ...
    <v-dialog v-model="dialog" max-width="650">
      ...
      <v-card rounded="lg" class="pa-1">
        ...
        <v-card-title class="py-4">
          ...
          <span class="text-h6 font-weight-bold">Supprimer une offre</span>
        </v-card-title>
        ...
        <v-card-text class="pt-6">
          ...
          <!-- Liste des entreprises utilisant cette offre -->
          <v-card
            class="mb-2"
            v-if="props.typeOffre.entrepriseOffres.length > 0"
          >
            <v-list disabled>
              <v-subheader>
                Vous devez d'abord supprimer cette offre des entreprises suivantes :
              </v-subheader>

              <v-list-item
                v-for="(ent, i) in props.typeOffre.entrepriseOffres"
                :key="i"
              >
                <v-list-item-title>
                  {{ ent.entreprise.nom }}
