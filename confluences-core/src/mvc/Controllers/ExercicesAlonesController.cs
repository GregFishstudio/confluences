...
namespace mvc.Controllers
{
    public class ExercicesAlonesController : Controller
    {
        ...
        public async Task<IActionResult> Create(
            [Bind("ExerciceId,ExerciceDate,ExerciceName,ExerciceLink,AudioLink,AudioLinkSecond,AudioLinkThird,CorrectionDate,CorrectionLink,IsHomeworkAdditionnal,TeacherId,HomeworkV2id,VideoLink")] ExercicesAlone exercice, 
            [FromForm] List<IFormFile> files, 
            [FromForm] List<IFormFile> filesCorrection, 
            [FromForm] IFormFile fileAudio, 
            [FromForm] IFormFile fileAudioSecond, 
            [FromForm] IFormFile fileAudioThird, 
            [FromForm] IFormFile fileVideo)
        {
            // Préparation de l'appel à l'API
            string accessToken = await HttpContext.GetTokenAsync("access_token");
            HttpClient client = new HttpClient();
            ...
            if (ModelState.IsValid)
            {
                ...
                if (fileAudioSecond != null)
                {
                    ...
                    var stream = fileAudioSecond.OpenReadStream();
                    content = new StreamContent(stream);
                    content.Headers.ContentDisposition = new ContentDispositionHeaderValue("form-data")
                    {
                        Name = "files",
                        FileName = Path.GetExtension(fileAudioSecond.FileName)
                    };
                    form.Add(content);

                    HttpResponseMessage responseTheory = await client.PostAsync(_configuration["URLAPI"] + "api/UploadFiles/SaveExercice", form);

                    exercice.AudioLinkSecond = await responseTheory.Content.ReadAsStringAsync();

                }
                if (fileAudioThird != null)
                {
                    // Préparation de la requête update à l'API
                    MultipartFormDataContent form = new MultipartFormDataContent();
                    HttpContent content = new StringContent("files");
                    ...
                    var stream = fileAudioThird.OpenReadStream();
                    content = new StreamContent(stream);
                    content.Headers.ContentDisposition = new ContentDispositionHeaderValue("form-data")
                    {
                        Name = "files",
                        FileName = Path.GetExtension(fileAudioThird.FileName)
                    };
                    form.Add(content);

                    HttpResponseMessage responseTheory = await client.PostAsync(_configuration["URLAPI"] + "api/UploadFiles/SaveExercice", form);

                    exercice.AudioLinkThird = await responseTheory.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> Edit(
            int id, 
            [Bind("ExerciceId,ExerciceDate,ExerciceName,ExerciceLink,AudioLink,AudioLinkSecond,AudioLinkThird,CorrectionDate,CorrectionLink,IsHomeworkAdditionnal,TeacherId,HomeworkV2id,VideoLink")] ExercicesAlone exercicesAlone, 
            [FromForm] List<IFormFile> files, 
            [FromForm] List<IFormFile> filesCorrection, 
            [FromForm] IFormFile fileAudio, 
            [FromForm] IFormFile fileAudioSecond, 
            ...
            [FromForm] IFormFile fileVideo)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (fileAudioSecond != null)
                {
                    ...
                    exercicesAlone.AudioLinkSecond = await responseTheory.Content.ReadAsStringAsync();
                ...
                if (fileAudioThird != null)
                {
                    ...
                    exercicesAlone.AudioLinkThird = await responseTheory.Content.ReadAsStringAsync();
