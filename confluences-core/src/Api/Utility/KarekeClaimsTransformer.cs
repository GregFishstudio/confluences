        ...
        public async Task<ClaimsPrincipal> TransformAsync(ClaimsPrincipal principal)
        {
            if (principal.Identity.IsAuthenticated)
            {
                ...
                if (principal.FindFirst("role") == null && userId != null)
                {
                    ...
                    using (var context = new ConfluencesDbContext())
                    {
                        // Faire depuis API
                        var roles = context.UserRoles.Where(a => a.UserId == userId.Value).Select(r => r.RoleId);
                        foreach (var role in roles)
                        {
                            ((ClaimsIdentity)principal.Identity).AddClaim(new Claim(JwtClaimTypes.Role, role.ToString(),
